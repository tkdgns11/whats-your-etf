...
CREATE TABLE etf_compositions (
    ...
    CONSTRAINT fk_component FOREIGN KEY (component_ticker)
...
CREATE TABLE strategies (
    ...
    portfolio_id   BIGINT REFERENCES portfolios(portfolio_id),
    simulation_id  BIGINT REFERENCES simulations(simulation_id),
    feedback_id    BIGINT REFERENCES portfolio_ai_feedback(id),
    name           VARCHAR(100),
    description    TEXT,
    -- 저장 시점 스냅샷
...
[매핑 방법]
  ...
  3. 매월 1회 구성종목 갱신 시 etf_sector_breakdown 함께 갱신
...
[뉴스 영향력 분석 활용]
  ...
  → etf_sector_breakdown 기반으로 관련 ETF 자동 추출
```
...
| 3 | **포트폴리오 AI 피드백** | 포트폴리오 → 헤드라인 + 키워드 + 분석 | LLM 프롬프팅 |
...
```
[목적]
  "트럼프 관세 뉴스가 떴다 → 어떤 ETF가 영향받는가?"
  뉴스를 읽고 → 산업 매핑 → ETF 매핑 → 실제 주가로 검증

[2-Step 분석 방식]
  기존: 뉴스 → ETF 직접 매핑 (LLM 예측, 부정확)
  개선: 뉴스 → 산업 매핑 (LLM) → ETF 매핑 (데이터 기반) → 실제 주가 검증

[처리 흐름]

  ── Step 1: 뉴스 발행 시점 (10분 배치) ──

    1. Google News RSS + 네이버 뉴스로 경제/산업 뉴스 수집
       - 산업분류(group_code) 기반 키워드로 검색 (22개 산업, 250+ 키워드)
       - 본문 크롤링 가능 언론사: hankyung, sedaily, edaily, news1, etoday, bizwatch
       - 본문 불가 언론사: RSS snippet 사용

    2. LLM에 뉴스 전달 → 산업 매핑:
       "이 뉴스가 어떤 산업에 관한 것인지 분석해주세요."
       → news_industry_influence 저장 (산업코드 + 관련도 + 감성)

    3. 관심 ETF 보유 사용자에게 즉시 알림 발송
       → 산업 → ETF 매핑 → 관심종목 등록 사용자 → FCM 푸시

  ── Step 2: 장 마감 후 (일 1회 배치) ──

    1. 뉴스 발행 후 실제 ETF 가격 변동률 조회
       - 장중 뉴스 → 당일 장 마감 후 검증
       - 장 마감 후 뉴스 → 다음 거래일 장 마감 후 검증

    2. ETF 영향도 계산
       - industry → ETF 매핑 (etf_sector_breakdown 기반)
       - 영향도 = (산업 관련도 × 0.5) + (실제 변동률 기여도 × 0.5)

    3. 타임라인 텍스트 생성 (LLM)
       - timeline_title: "연준 기준금리 동결 발표" (20자)
       - timeline_summary: "시장 예상치 부합, 기술주 중심 반등세" (50자)

    4. news_etf_influence 저장 (is_verified = TRUE)
       → 과거 데이터는 모두 실제 주가 기반으로 검증됨

[본문 처리 전략]
  - 모든 뉴스: 제목 + RSS snippet → content_summary 저장
  - 본문 크롤링 가능 언론사: content 컬럼에 본문 추가 저장
  - LLM 분석 입력: 제목 + content_summary (본문 유무 무관)
  - 사용자 UI: 본문 있으면 상세 표시, 없으면 "원문 보기" 외부 링크

[데이터 테이블]
  - news_source: 언론사 관리 (본문 크롤링 가능 여부 자동 관리)
  - news_article: 뉴스 기사 (제목, 본문, 요약)
  - news_industry_influence: 1차 분석 (뉴스 → 산업)
  - news_etf_influence: 2차 분석 (검증된 ETF 영향)
...
#### 뉴스 (Google News RSS + 네이버)
```
[과거 뉴스 — 초기 적재]
  BigKinds (bigkinds.or.kr): 경제/산업 CSV 다운로드 (~50만건)

[일일 뉴스 — 서비스 운영 중]
  Google News RSS + 네이버 뉴스 검색 API
  → 산업분류(group_code) 기반 키워드 검색 (22개 산업, 250+ 키워드)
  → 10분마다 수집 → 본문 크롤링 → LLM 산업 매핑 → DB 저장

[뉴스 소스(언론사) 관리 — news_source 테이블]
  본문 크롤링 가능 여부를 언론사별로 관리

  ✅ 본문 크롤링 가능 (6개):
     hankyung.com (한국경제), sedaily.com (서울경제), edaily.co.kr (이데일리)
     news1.kr (뉴스1), etoday.co.kr (이투데이), bizwatch.co.kr (비즈워치)

  ❌ 본문 크롤링 불가 (봇 차단 등):
     mk.co.kr (매일경제), yna.co.kr (연합뉴스), biz.chosun.com (조선비즈)
     mt.co.kr (머니투데이), fnnews.com (파이낸셜뉴스) 등

  자동화 로직:
  - 크롤링 연속 실패 5회 → is_content_available = FALSE
  - 1일 1회 비활성 언론사 재테스트 → 성공 시 재활성화

[본문 처리 전략]
  - 모든 뉴스: 제목 + RSS snippet → content_summary 저장
  - 본문 크롤링 가능: content 컬럼에 본문 추가 저장
  - LLM 분석: 제목 + content_summary 사용 (본문 유무 무관)
  - 사용자 UI: 본문 있으면 상세 표시, 없으면 "원문 보기" 외부 링크
```
...
| 역할 | 담당 | 주요 업무 |
|---|---|---|
| **BE - ETF/전략** | 권택민 | ETF API, 시뮬레이션, 전략 CRUD, KIS API 연동, 실시간 지표 |
...
[나의전략]
  - 저장된 전략 목록 조회
  - 전략 상세 조회: 그래프(과거/현재) + 타임라인 + 뉴스
  - 전략 수립 이후 움직임 조회
  - 전략 수정 (새로운 포트폴리오 생성)
  - 시뮬레이션 결과 차트 / 이후 추세 차트 분리
...
[AI 검증]
  - 일일 경제 뉴스 크롤러: Google News RSS, 10분마다 수집
  - 뉴스 기반 영향력 매핑: 뉴스 → 구성종목 → ETF 클러스터 하이라이트
...
### 4주차: AI 포트폴리오 진단 + 통합
```
권택민     : 전략 저장 API, 실시간 지표(PER/배당/섹터) API
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
산출물: AI 포트폴리오 진단, 뉴스 영향력 맵, 전략 저장
```

### 5주차: 전략 추적 + 고도화
```
권택민     : 전략 추적 배치, 전략 비교 API, 성능 최적화 시작
윤상훈     : 전략 추적 로직, 알림 기능 (상장폐지/상장/리밸런싱)
신우혁     : 트리맵 보기 구현, 클러스터 맵 고도화 (애니메이션)
이새롬     : 시뮬레이션 비교 기능, 슬라이더 고도화
김준규     : 나의전략 상세 (추적 차트), ETF 상세 고도화
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
